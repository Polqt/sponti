-- Migration: add offset_count parameter to get_trending_locations so the
-- Flutter client can paginate the explore feed without fetching all rows.
-- Additive and safe to deploy while the app still passes offset_count = 0.

DROP FUNCTION IF EXISTS public.get_trending_locations(
  TEXT, TEXT, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN, INTEGER
);
...
AS $$
  ...
  ORDER BY
    ...
    CASE WHEN nf.value = 'popular'  THEN COALESCE(fc.favorite_count, 0) END DESC NULLS LAST,
